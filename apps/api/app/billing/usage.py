"""In-process + Postgres usage counters for free-tier chat caps."""
...
async def record_chat_and_check(user_id: str, plan: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        if is_db_available():
            ...
            if not allowed:
                return {"allowed": False, "used": used, "limit": limit, "plan": p}
            used = await increment_chat_usage(user_id)
            return {"allowed": True, "used": used, "limit": limit, "plan": p}
    except Exception:  # noqa: BLE001
        pass

    allowed, used0, limit = plan_allows_chat(p, used)
    if not allowed:
        return {"allowed": False, "used": used0, "limit": limit, "plan": p}
    used_n = _mem_inc(user_id)
    return {"allowed": True, "used": used_n, "limit": limit, "plan": p}
